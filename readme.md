...
ServicePortResponsibilityAuth Service5000User authentication & JWT issuanceRestaurant Service5001Restaurants, menus, ordersUtils Service5002File/image uploads (Cloudinary)Realtime Service5004Socket.io server for live eventsRider Service5005Rider profiles, availability, order pickupAdmin Service5006Verification of restaurants & ridersFrontend5173React + Vite client
...
envMONGODB_URL=mongodb+srv://<user>:<password>@cluster0.mongodb.net/cloudKitchenX?appName=Cluster0
PORT=5005
...
bash# For each service (auth, restaurant, utils, realtime, rider, admin, frontend)
cd <service-folder>
npm install
...
User signs up/logs in via Google OAuth, selects a role (customer, rider, seller).
Riders & restaurants submit profiles for verification.
Admin reviews and verifies pending riders/restaurants.
Customers browse restaurants, place orders, and pay via Razorpay.
Restaurant accepts the order, triggering a real-time notification to nearby online riders.
Rider accepts the order, picks it up, and shares live location.
Customer tracks the order in real time on a live map until delivery.
